                        ...
                        <div class="row g-3">
                            <div class="col-md-6">
                                ...
                                <select id="service_type_id" class="form-select @error('service_type_id') is-invalid @enderror" name="service_type_id" required>
                                    ...
                                    @foreach($serviceTypes as $type)
                                        <option value="{{ $type->id }}" data-name="{{ $type->name }}" {{ old('service_type_id') == $type->id ? 'selected' : '' }}>
                                            {{ $type->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-12" id="address-field" style="display:none;">
                                ...
                                <textarea id="address" class="form-control @error('address') is-invalid @enderror"
                                          name="address" rows="2"
                                          placeholder="Full property address shown on the booking confirmation">{{ old('address') }}</textarea>
                                @error('address')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
                            ...
                            <div class="col-md-6" id="bhk-type-field" style="display:none;">
                                ...
                                <select id="bhk_type" class="form-select @error('bhk_type') is-invalid @enderror" name="bhk_type">
                                    ...
                                    <option value="Studio/1 RK" {{ old('bhk_type') === 'Studio/1 RK' ? 'selected' : '' }}>Studio/1 RK</option>
                                    <option value="1 BHK" {{ old('bhk_type') === '1 BHK' ? 'selected' : '' }}>1 BHK</option>
                                    <option value="2 BHK" {{ old('bhk_type') === '2 BHK' ? 'selected' : '' }}>2 BHK</option>
                                    <option value="3 BHK" {{ old('bhk_type') === '3 BHK' ? 'selected' : '' }}>3 BHK</option>
                                    <option value="4 BHK" {{ old('bhk_type') === '4 BHK' ? 'selected' : '' }}>4 BHK</option>
                                    <option value="5 BHK" {{ old('bhk_type') === '5 BHK' ? 'selected' : '' }}>5 BHK</option>
                                    <option value="Villa/Duplex" {{ old('bhk_type') === 'Villa/Duplex' ? 'selected' : '' }}>Villa/Duplex</option>
                                </select>
                                @error('bhk_type')
                                    <span class="invalid-feedback">{{ $message }}</span>
                                @enderror
