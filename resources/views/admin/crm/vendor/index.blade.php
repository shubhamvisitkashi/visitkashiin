                    ...
                    <div class="card-header">
                        <div class="row">
                            ...
                            <div class="col-md-6" style="padding-bottom:10px">
                                <x-add-btn route="{{ route('vendor.create') }}" />
                            </div>
                            <div class="col-md-12">
                                <form action="{{ route('vendor.index') }}" id="search_form">
                                    <div class="row">
                                        <div class="col-3">
                                            ...
                                            <div class="input-group">
                                                <select class="form-control" name="search_service" id="search_service">
                                                    ...
                                                    @foreach ($vendor_services as $vendor_service)
                                                        <option
                                                            value="{{ $vendor_service->id }}"{{ $vendor_service->id == $search_service ? 'selected' : '' }}>
                                                            {{ $vendor_service->name }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
                                ...
                                @foreach ($vendor_services as $vendor_service)
                                    <div class="lead-status-card">
                                        <a href="{{ route('agent.index') }}">
                                            <div
                                                class="bg-total bg-lead py-2 ps-3 pe-2 d-flex align-items-center justify-content-between">
                                                <h3>Total {{$vendor_service->name}}</h3>
                                                <span>{{App\Models\Vendor::wherejsonContains('service_ids',''.$vendor_service->id)->when(Auth::guard('admin')->user()->id != '1', function ($query) {
