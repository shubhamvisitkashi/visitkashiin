...
.kpi-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(215px,1fr)); gap:18px; margin-bottom:22px; }

.kpi-card {
  background:var(--card); border-radius:var(--r); border:1px solid var(--border);
  box-shadow:var(--shadow); padding:20px 22px 20px 24px; display:flex; align-items:center;
  gap:16px; position:relative; overflow:hidden;
  transition:transform var(--t), box-shadow var(--t), border-color var(--t);
}
.kpi-card:hover { transform:translateY(-4px); box-shadow:0 14px 34px rgba(0,0,0,.12); border-color:color-mix(in srgb, var(--kpi-color, var(--indigo)) 35%, var(--border)); }
...
/* Soft left accent bar (replaces the old bottom bar) — reads better against
   the icon chip and gives each card a clearer color identity at a glance. */
.kpi-card::after {
  content:''; position:absolute; left:0; top:0; bottom:0; width:4px;
  background:var(--kpi-color, var(--indigo));
}
/* Faint tinted wash using the card's own icon color, so the card doesn't
   look flat/empty on wide screens where the body text hugs the left edge. */
.kpi-card::before {
  content:''; position:absolute; inset:0; pointer-events:none; opacity:.5;
  background:linear-gradient(120deg, var(--kpi-bg, transparent) 0%, transparent 60%);
}
.kpi-icon {
  width:52px; height:52px; border-radius:14px; display:flex; align-items:center;
  justify-content:center; font-size:1.5rem; flex-shrink:0;
  background:var(--kpi-bg, #EEF2FF); position:relative; z-index:1;
...
.kpi-value { font-size:1.85rem; font-weight:900; color:var(--text); line-height:1.1; letter-spacing:-.01em; overflow-wrap:break-word; }
.kpi-sub   { font-size:.72rem; color:var(--muted); margin-top:6px; display:flex; align-items:center; gap:5px; flex-wrap:wrap; overflow-wrap:break-word; }
